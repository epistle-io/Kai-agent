...
async def _get_account_async():
    async with websockets.connect(WS_URL, ping_interval=30) as ws:
        ...
        bal = br.get("balance", {})
        return {"balance": bal.get("balance",0), "equity": bal.get("balance",0),
                "currency": bal.get("currency","USD"), "profit": 0,
                "free_margin": bal.get("balance",0), "leverage": 100,
                "login": data.get("loginid",""), "server": "Deriv-Demo" if DERIV_DEMO else "Deriv-Live"}
...
async def _place_trade_async(symbol, direction, amount, sl_pips, tp_pips):
    ...
    async with websockets.connect(WS_URL, ping_interval=30) as ws:
        ...
        await ws.send(json.dumps({"proposal":1, "amount": amount, "basis":"stake",
            "contract_type": ctype, "currency":"USD", "duration":1, "duration_unit":"d",
            "symbol": sym, "multiplier":100,
            "limit_order":{"stop_loss": sl_pips, "take_profit": tp_pips}}))
        pr = json.loads(await ws.recv())
        if pr.get("error"): return {"success": False, "error": pr["error"]["message"]}
        pid = pr.get("proposal", {}).get("id")
        if not pid: return {"success": False, "error": "No proposal ID"}
        await ws.send(json.dumps({"buy": pid, "price": amount}))
        br = json.loads(await ws.recv())
        if br.get("error"): return {"success": False, "error": br["error"]["message"]}
        c = br.get("buy", {})
        return {"success": True, "ticket": c.get("contract_id"), "symbol": symbol,
                "direction": direction, "lot": amount, "time": datetime.now().isoformat()}
